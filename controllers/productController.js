...
export async function createProduct(req, res) {
  try {
    ...
    const created_by =
      req.user && req.user.name ? req.user.name : 'Nomaʼlum'
...
export async function getProductsByDate(req, res) {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM products ${condition} ORDER BY product_date DESC`
    )

    res.json(result.rows)
  } catch (error) {
    console.error('GET PRODUCTS BY DATE ERROR:', error)
    res.status(500).json({ message: 'Sanaga ko‘ra olishda xatolik' })
  }
}